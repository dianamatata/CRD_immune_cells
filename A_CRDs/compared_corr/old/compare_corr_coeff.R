...
COR_c1_vs_c2 <- function(COR_c1,COR_c2,N1,N2) {
  COR_c1_vs_c2 <- data.frame(matrix(ncol = 6, nrow = 0))
  colnames(COR_c1_vs_c2) <- c("peak1", "peak2", "corr_c1","corr_c2", "fisher_statistic", "pvalue")
  for (i in seq(1,dim(COR_c1)[1])){
    a <- cocor.indep.groups(COR_c1$V7[i], COR_c2$V7[i], N1, N2, alternative = "two.sided",
                            test = "all", alpha = 0.05, conf.level = 0.95, null.value = 0,
                            data.name = NULL, var.labels = NULL, return.htest = FALSE)
    COR_c1_vs_c2 <- rbind(COR_c1_vs_c2,c(COR_c1$V2[i],COR_c1$V3[i],COR_c1$V7[i],COR_c2$V7[i],a@fisher1925$statistic,a@fisher1925$p.value))
    colnames(COR_c1_vs_c2) <- c("peak1", "peak2", "corr_c1","corr_c2", "fisher_statistic", "pvalue")
    COR_c1_vs_c2
  }
}
