...
wdir="/Users/dianaavalos/switchdrive/CRD_immune/plot_CRD_genes_main_fig2"
setwd(wdir)
...
rna_file <- c('EGAD00001002675_chr5.txt', 'EGAD00001002674_chr5.txt', 'EGAD00001002671_chr5.txt') # how was this generated?
...
chr5_cell_genes_info <- function (path_RNA,rna_file,cell) {
  ...
  genes_exp=rowMeans(genes_chr5[,7:dim(genes_chr5)[2]]) # TODO: error rowMeans should be here, so compute the mean after
...
for (cell in c('neut','mono','tcell')){
  ...
  CRD_count=5 # this is a filter on the CRD nodes
   for (CRD_count in c(3)){
    ...
    genes_chr5_summary=chr5_cell_genes_info(path_RNA,rna_file,cell) # TODO: here change values
    ...
    genes_chr5_filtered_associated <- inner_join(genes_chr5_filtered,ASSOC, by=c("gene"="V1")) # 17 genes are expressed and associated to a CRD (5%)
    genes_chr5_filtered_associated_withCRD_in_plot <- inner_join(genes_chr5_filtered,ASSOC2, by=c("gene"="V1")) # 5 genes are expressed and have CRD-associations with the CRDs in the plot 
    ...
    #new
    # associated_genes_100filter=genes_chr5_filtered_associated_withCRD %>% filter(mean>=0)
...
for (cell in c('neut','mono','tcell')){
  ...
  ggplot() + 
    ...
    # add MODS
      ...
      MODSs <- MODS[MODS$N_REG > 1 & MODS$MOD == 1 & MODS$COUNT > 5, ] %>% filter(RIDX > section & LIDX < section+1000) # create a subset of MODS in the window 1000-2000
      CRDs_in_plot <- unique(MODSs$UUID)
      CRDs_in_plot <- unique(MODSs_M$UUID)
      
      # PLOT CRD
      par(fig=c(0.00, 1.00, 0.00, 1.00), new=FALSE) # c(x1, x2, y1, y2)
