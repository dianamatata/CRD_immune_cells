#**************************** PLOTS   Hi-C analysis just for gene-CRD associations
...
compute_hic_validated <- function(PCHiC, array_gene_CRDS_data){
  ...
  genebed <- GRanges(seqnames=array_gene_CRDS_data$phenotype_ID_chr,ranges=IRanges(start=array_gene_CRDS_data$phenotype_ID_start, end=array_gene_CRDS_data$phenotype_ID_end)) 
  CRDbed <- GRanges(seqnames=array_gene_CRDS_data$CRD_ID_chr,ranges=IRanges(start=array_gene_CRDS_data$CRD_ID_start, end=array_gene_CRDS_data$CRD_ID_end))
  
  #fwd
  x = findOverlaps(baitbed,genebed)
  y = findOverlaps(oebed,CRDbed)
  tmp = rbind(as.data.frame(x),as.data.frame(y))
  validated.fwd = tmp[which(duplicated(tmp)),] # TODO: why duplicated
  
  #bwd
  z = findOverlaps(baitbed,CRDbed)
  a = findOverlaps(oebed,genebed)
  tmp = rbind(as.data.frame(x),as.data.frame(y)) 
...
compute_HiC_column_in_array_CRD_genes <- function(PCHiC, array_CRD_genes,validated,cell_type){
  
  hic_validated = rep(1,nrow(PCHiC))
  
  for(i in 1:nrow(validated)){
    currenthic = validated$queryHits[i]
    currentpval = array_CRD_genes$bwd_pval[validated$subjectHits[i]]
    if(currentpval<hic_validated[currenthic]){
      hic_validated[currenthic] = currentpval
    }
  } # TODO: how does this work? can we check the pipeline?
  
  array_CRD_genes_validated = rep(0,nrow(array_CRD_genes))
  for(i in 1:nrow(validated)){
    currenthic = validated$queryHits[i]
    ...
    if(currentHiCScore>array_CRD_genes_validated[currentmap]){
      array_CRD_genes_validated[currentmap] = currentHiCScore
    }
  }
  array_CRD_genes_validated
}


compute_ratio_hic_array_CRD_genes <-function(CRDmindist,CRDmaxdist,cutoff=5){
  
  up = sum(abs(array_CRD_genes$distance)>=CRDmindist & abs(array_CRD_genes$distance)<CRDmaxdist & array_CRD_genes_validated>cutoff,na.rm=T)
  down = sum(abs(array_CRD_genes$distance)>=CRDmindist & abs(array_CRD_genes$distance)<CRDmaxdist,na.rm=T)
  up/down*100
}
...
PCHiC = fread('/Users/dianaavalos/Programming/HiC_nov20/PCHiC_peak_matrix_cutoff5.tsv', header=TRUE, select=c("#baitChr","baitStart","baitEnd","oeChr","oeStart","oeEnd","dist","Mon","Neu","nCD4" ))
# head(array_CRD_genes)
...
for(data_type in data_types){
  for(cell_type in cell_types){
    for(condition in conditions){
      ...
      file_array_CRD_genes=paste0(path_CRD_genes,data_type,'_',cell_type ,'_',condition,'_conditional.txt.gz')
      print(name_condition)
      
      # download files
      allCRDs = fread(file_CRD,header=F)
      array_CRD_genes = read.table(file_array_CRD_genes, head=F, stringsAsFactors=F)
  
      colnames(array_CRD_genes) = c("phenotype_ID","phenotype_ID_chr","phenotype_ID_start","phenotype_ID_end","phenotype_ID_strand",
      ...
      array_CRD_genes = array_CRD_genes[array_CRD_genes$phenotype_ID %in% genelist,]
      array_CRD_genes = array_CRD_genes[order(array_CRD_genes[,2],array_CRD_genes[,3]),]
      
      # MAIN
      nb_CRD_not_associated = nrow(allCRDs) - length(unique(array_CRD_genes$CRD_ID))
      nb_genes_not_associated = length(unique(genelist)) - length(unique(array_CRD_genes$phenotype_ID))
      
      # check sizes
      print(paste0(length(genelist),"   ", nb_genes_not_associated))
      print(paste0(nrow(allCRDs),"   ", nb_CRD_not_associated))
      
      # consider the different TSS for each gene in the HiC analysis?  ---------------------------------------------
      validated=compute_hic_validated(PCHiC, array_CRD_genes)
      
      array_CRD_genes_validated=compute_HiC_column_in_array_CRD_genes(PCHiC, array_CRD_genes,validated,cell_type)
      array_CRD_genes$HIC=array_CRD_genes_validated
      # head(array_CRD_genes)
      ...
      for(i in 1:(length(crd_dist_bins)-1)){
        mat_hic[i,1] = compute_ratio_hic_array_CRD_genes(crd_dist_bins[i],crd_dist_bins[i+1])
      }
      ...
      ga <- ggplot(toplot, aes(x = dist, y = counts))+ ggtitle("HiC support for gene-CRD associations") +
      ...
      CRD_IDs = unique(array_CRD_genes$CRD_ID)
      IDXs = c()
      CRDdistance = c()
      CRDdistance_gene1=c()
      CRDdistance_gene2=c()
      ...
      for(i in 1:length(CRD_IDs)){
        # cat(i,"\n")
        idx = which(array_CRD_genes$CRD_ID == CRD_IDs[i])
        if(length(idx)>1){
          for(k in 1:(length(idx)-1)){
            for(l in (k+1):length(idx)){
              geneA = array_CRD_genes$phenotype_ID[idx[k]]
              geneB = array_CRD_genes$phenotype_ID[idx[l]]
              ...
              if(length(myhit)>0){
                ...
                IDXs = c(IDXs,myhit)
                tmp = mean(abs(array_CRD_genes$distance[idx[k]]),abs(array_CRD_genes$distance[idx[l]])) # distance of the gene to the CRD CRD_IDs[i]
                tmp2 = mean(array_CRD_genes_validated[idx[k]],array_CRD_genes_validated[idx[l]])
                hicsupport = c(hicsupport,tmp2)
                
                CRDdistance = c(CRDdistance,tmp) # mean distance with the 2 genes
                CRDdistance_gene1 = c(CRDdistance_gene1,abs(array_CRD_genes$distance[idx[k]])) # dist of gene 1
                CRDdistance_gene2 = c(CRDdistance_gene2,abs(array_CRD_genes$distance[idx[l]])) # dist of gene 2
              }
            }
          }
        }
...
# add TSS to array_CRD_genes
array_CRD_genes$phenotype_ID_2 <- sapply(array_CRD_genes$phenotype_ID, function(x)  strsplit(x,"[.]")[[1]][1], USE.NAMES=FALSE)
df1 <- getBM(attributes = c( "chromosome_name", "transcript_start", "transcript_end", "strand",  "ensembl_gene_id",
                             "ensembl_transcript_id", "external_gene_name", "transcription_start_site"),
             filters = "ensembl_gene_id",
             values = array_CRD_genes$phenotype_ID_2,
             mart = ensembl,
             useCache = FALSE)
# problem: many transcription_start_site per gene
# for each gene in df1, add info from array_CRD_genes about CRD
new_array_CRD_gene <- data.frame()

gene <- unique_genes[1]
for (gene in unique(df1$ensembl_gene_id)){
  sub_df1 <- df1 %>% filter(ensembl_gene_id==gene)
  sub_array_CRD_genes <- array_CRD_genes  %>% filter(phenotype_ID_2==gene)
  # gene can be associated with many CRDs
  sub_df1$assumed_TSS <- sub_array_CRD_genes[1,]$phenotype_ID_start
  
  if (dim(new_array_CRD_gene)[1]==0) {
    new_array_CRD_gene <- sub_df1
  }  else {
    new_array_CRD_gene <- rbind(new_array_CRD_gene,sub_df1)
  }
}

new_array_CRD_gene$dist=abs(new_array_CRD_gene$assumed_TSS - new_array_CRD_gene$transcription_start_site)
hist(dist)

sub_df1$CRD_ID_chr <- sub_array_CRD_genes$CRD_ID_chr
sub_df1$CRD_ID_start <- sub_array_CRD_genes$CRD_ID_start
sub_df1$CRD_ID_end <- sub_array_CRD_genes$CRD_ID_end


# for each line of sub_df1 run validated
#mean per gene somehow? 
array_gene_CRDS_data <- sub_df1
validated2 <- compute_hic_validated(PCHiC, array_gene_CRDS_data)

# TODO: check that here we only take the TSS. we only have one value going out of the CRD-gene file: /home/users/a/avalosma/scratch/5_CRDgene/merged_TH
# when we have 4_with PC, we already have only start point, lets go before PC to have the genes starting point
