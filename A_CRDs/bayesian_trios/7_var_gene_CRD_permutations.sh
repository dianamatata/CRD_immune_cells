...
for data_type in  'methyl' 'hist' ; do
		...
		BED=$BEDDIR/${data_type}_${cell_type}.pc1.sorted.txt.gz
			for k in $(seq 1 $K); do
				OUT1=$OUTDIR/mapping_$permuts/${data_type}_${cell_type}_${window}_${module}CRD_gene_var_chunk$k
				cmd1="QTLtools cis --vcf $VCF --bed $BED --permute $permuts --window $window --chunk $k $K --out ${OUT1}.txt"
				wsbatch -J 1000permuts.job --partition=shared-cpu --time=04:00:00 \
				-o 1000permuts.out -e 1000permuts.err --wrap="$cmd1"
			done
        done
done
...
for data_type in  'methyl' 'hist' ; do
	for cell_type in 'neut' 'mono' 'tcell' ; do
			name=${data_type}_${cell_type}_${window}_CRD_gene_var
