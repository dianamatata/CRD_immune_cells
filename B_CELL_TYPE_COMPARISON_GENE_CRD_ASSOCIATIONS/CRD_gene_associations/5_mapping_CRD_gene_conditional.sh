...
OUTDIR=$DATADIR/mapping_gene_CRDs_conditional

mkdir -p $OUTDIR $DATADIR/merged_TH
...
for data_type in  'methyl' 'hist' ; do
        for cell_type_quantifM in 'neut'  'mono' 'tcell' ; do
        		...
        		MOD=$DATADIR/quantify_ALL/${name}.ALLchr.${module}.txt.gz
	                ...
	                TH=$DATADIR/mapping_gene_CRDs/significants/FDR_0.05_${name}_${module}_mapping_CRD_gene_permuts.thresholds.txt
			for k in $(seq 1 $K); do
				OUTFILE1=$OUTDIR/${name}_CRD_gene_chunk$k
				cmd="QTLtools cis --vcf $MOD --bed $RNA --mapping $TH --chunk $k $K --out $OUTFILE1.txt"	
				eval $cmd
			done
		done
	done
...
for data_type in  'methyl' 'hist' ; do
        for cell_type_quantifM in 'neut'  'mono' 'tcell' ; do
                for cell_type_CRD in 'neut'  'mono' 'tcell' ; do
                        name=${data_type}_${cell_type_quantifM}_vs_${cell_type_CRD}
                        module='mean'
                        cat $OUTDIR/${name}_*.txt | gzip -c > $DATADIR/merged_TH/${name}_ALL.txt.gz
                done
        done
done
